...
import torch
from .euclidean import euc_sqdistance
...
def LorentzNorm(x):       
    ...
    #print(nonclampledlorNorm)
    #print( lorNorm)
    #exit() 
    lNorm = lorNorm.sqrt()            
    if torch.isnan(lorNorm).any():    
        print(eucNorm)        
        print('is nan')    
        exit()
...
def expmapH(e, u, c):
    ...
    sqrt_c = c ** 0.5
    u_norm = LorentzNorm(u).unsqueeze(1)#.clamp_min(MIN_NORM).unsqueeze(1)
    #lam = 2.0/(1-c*torch.pow(e_norm,2))
    #print(sqrt_c.size())
    #print(u_norm.size())
    #print(e.size())
    #rr = cosh(sqrt_c * u_norm.unsqueeze(1)) * e
    #exit()
...
def projectH(x, c):
    ...
    Returns:
        torch.Tensor with projected hyperbolic points.
    """    
    norm = LorentzNorm(x)#.clamp_min(MIN_NORM)
    eps = BALL_EPS[x.dtype]
    maxnorm = (1 - eps) / (c ** 0.5)
    #print(x.size())    
    #print(norm.size())
    #print(maxnorm.squeeze(1).size())
    #exit()
...
def busemann(p,x, keepdim=True):
    ...
    if x.shape[0] == p.shape[0]:
        num = (torch.norm(p - x, dim=-1, keepdim=True) ** 2).clamp_min(MIN_NORM)
        ans = torch.log((den/num).clamp_min(MIN_NORM))
    else:
        num = euc_sqdistance(p, x, eval_mode=True).clamp_min(MIN_NORM)
        ans = torch.log((den.view(-1,den.shape[0])/num).clamp_min(MIN_NORM)).t()
    if not keepdim:
        ans = ans.squeeze(-1)
    return ans

def busemann_distance(x, y, c, eval_mode=False):
    """Hyperbolic distance on Poincare balls with varying curvatures c.
    Args:
        x: torch.Tensor of size B x d with hyperbolic queries
        y: torch.Tensor with hyperbolic queries, shape n_entities x d if eval_mode is true else (B x d)
    ...
    """
    py = y/y.norm(dim=-1, p=2, keepdim=True)
    px = x/x.norm(dim=-1, p=2, keepdim=True)
    
    bus_dist_x = busemann(py, x)
    bus_dist_y = busemann(px, y)
    
    if eval_mode:
        return bus_dist_x+bus_dist_y.t()
    else:
        return bus_dist_x+bus_dist_y
